    ...
    return (
        <div className="p-4 rounded-xl border border-gray-100 shadow-sm bg-white transition hover:shadow-md">
            <div className="flex items-start justify-between">
                <h3 className="text-base md:text-lg font-semibold text-gray-800">{index + 1}. {q.question}</h3>
                ...
                {Array.isArray(q.options) && q.options.length ? q.options.map((opt, i) => {
                    ...
                    return (
                        <li key={i} className={className}>
                            ...
                            <span className="ml-4 text-sm">{isCorrectOpt ? "✔️" : isUserChoice ? "✖️" : ""}</span>
                        </li>
                    );
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
                {isUnattempted ? <span className="text-gray-600">You did not attempt this question.</span> : (
                    <span className="text-gray-700">
                        Your answer: <span className={selected === correct ? "font-semibold text-green-700" : "font-semibold text-red-700"}>{selected}</span>
                    </span>
                )}
                {correct && <span className="text-gray-600">• Correct answer: <span className="font-medium text-green-800">{correct}</span></span>}
            </div>
        </div>
    );
}
